...
import com.ohgiraffers.blog_project.like.model.LikeDTO;
...
public class BlogController {
    ...
    @Autowired
    public BlogController(BlogService blogService, LikeService likeService) {
...
//    전체 조회할 화면을 가져온다.
    ...
    public String list(Model model){
        ...
        // 좋아요 수 조회 및 매핑
        // Map<좋아요를 값을 가져올 blogid, 좋아요 갯수>
        Map<Integer, Integer> likeCounts = new HashMap<>();
        // Map<k,v> : Map은 k와 v의 쌍으로 데이터를 저장하는 자료구조.
        // 제네릭 타입을 사용하여 키도  integer, 값도 integer 타입을 갖는다.

        //BlogDTO 는 List<BlogDTO> 타입으로 모든 블로그 포스트 데이터를 담는다.
        for(BlogDTO blog : blogDTOList){
            int likeCount = likeService.getLikeCountByBlogId(blog.getId());
            // likeService 의 getLikeCountByBlogId(blog.getId()) 를 가져오고 조회된 수를 likeCount 변수에 반영
            likeCounts.put(blog.getId(), likeCount);
        }
